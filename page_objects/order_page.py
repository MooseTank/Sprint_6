...
class OrderPage(BasePage):
    ...
    @allure.step('Кликнуть по варианту в списке станций метро')
    ...
    def select_date_by_keyboard_input(self):
        self.fill_input(OrderPageLocators.input_date).send_keys(TestData.test_data_user_1[5])
    ...
    def data_enter_first_form(self, test_data):
        self.wait_visibility_of_element(OrderPageLocators.input_name)
        self.click_on_element(OrderPageLocators.input_name)
        self.fill_input(OrderPageLocators.input_name, test_data[0])
        self.click_on_element(OrderPageLocators.input_lastname)
        self.fill_input(OrderPageLocators.input_lastname, test_data[1])
        self.click_on_element(OrderPageLocators.input_address)
        self.fill_input(OrderPageLocators.input_address, test_data[2])
        self.click_on_element(OrderPageLocators.input_metro)
        self.fill_input(OrderPageLocators.input_metro, test_data[3])
        self.click_on_element(OrderPageLocators.select_item_in_dropdown_metro)
        self.click_on_element(OrderPageLocators.input_phone)
        self.fill_input(OrderPageLocators.input_phone, test_data[4])
        self.click_on_element(OrderPageLocators.button_next)

    @allure.step('Заполнение второй чатси формы и окно подтверждения')
    def data_enter_second_form(self, test_data):
        self.wait_visibility_of_element(OrderPageLocators.input_date)
        self.click_on_element(OrderPageLocators.input_date)
        self.fill_input(OrderPageLocators.input_date, test_data[5])
        ...
        self.fill_input(OrderPageLocators.input_comment, test_data[6])
