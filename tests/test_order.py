...
class TestOrderPageOrder:
    ...
    @pytest.mark.parametrize('button, test_data', [(MainPageLocators.order_button_in_header, TestData.test_data_user_1), (MainPageLocators.order_button_in_main, TestData.test_data_user_2)])
    def test_order_all_fields(self, driver, button, test_data):
        order_page = OrderPage(driver)
        order_page.scroll_to_element(button)
        order_page.wait_visibility_of_element(button)
        order_page.click_on_element(button)
        order_page.data_enter_first_form(test_data)
        order_page.data_enter_second_form(test_data)
